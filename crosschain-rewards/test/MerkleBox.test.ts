...
import hardhat, { ethers, expect, upgrades, deployments } from 'hardhat';
import {
    ERC20WithPermitMock,
    ERC20WithPermitMock__factory,
    SonneMerkleDistributor,
    SonneMerkleDistributor__factory,
} from '../src/types';
...
const deployFixture = deployments.createFixture(async (hre, opts) => {
    ...
    const [deployer, funder, funder2, staker1, staker2] = await ethers.getSigners();
    const funder3 = new ethers.Wallet('0xa11a58ba8887796d1a7bc2a6107a98e0befd2b10c8846a39f39c05c6a976e725');

    // fake erc20
    const erc20 = await new ERC20WithPermitMock__factory().connect(deployer).deploy();
    ...
    // beacon proxy
    const beacon = await deployBeacon('SonneMerkleDistributor', hre);
    const beaconProxy = await deployBeaconProxy(
        'FakeDistributor',
        'SonneMerkleDistributor',
    ...
    const distributor = SonneMerkleDistributor__factory.connect(await beaconProxy.getAddress(), deployer);
    await distributor.grantRole(await distributor.MANAGER_ROLE(), funder.address);
    await distributor.grantRole(await distributor.FUNDS_ROLE(), funder.address);

    return [erc20, distributor] as const;
});

describe('SonneMerkleDistributor test', () => {
    ...
    beforeEach(async () => {
        ...
        [erc20, sonneMerkleDistributor] = await deployFixture();
    });
    afterEach(() => {
    ...
    describe('before a reward is created', () => {
        ...
        it('reverts when claiming from an unknown blockNumber', async () => {
            ...
            await expectRevert(
                sonneMerkleDistributor.connect(staker1).claim(unknownBlocknr, proof),
                'Reward not found',
            );
        });
    });
    ...
    describe('when creating a new reward', () => {
        it('emits NewMerkle event and deposits funds', async () => {
            ...
            const tx = await sonneMerkleDistributor
                .connect(funder)
                .addReward(1000, accountRoot, blocknr, unlockTime, 1000);
        ...
        it('reverts if merkleRoot is zero', async () => {
            ...
            await expectRevert(
                sonneMerkleDistributor.connect(funder).addReward(1000, ethers.ZeroHash, blocknr, unlockTime, 1000),
        ...
        it('reverts if insufficient balance', async () => {
            ...
            await expectRevert(
                sonneMerkleDistributor
                    .connect(funder)
                    .addReward(145192000000000000000001n, accountRoot, blocknr, unlockTime, 1000),
        ...
        it('reverts if amount is zero', async () => {
            ...
            await expectRevert(
                sonneMerkleDistributor.connect(funder).addReward(0, accountRoot, blocknr, unlockTime, 1000),
                'Invalid amount or insufficient balance',
            );
        });
    });
    ...
    describe('after creating a reward', () => {
        ...
        it('funder cannot withdraw', async () => {
            await expectRevert(
                sonneMerkleDistributor.connect(funder).withdrawFunds(blocknr, 50),
        ...
        it('funder cannot withdraw-all', async () => {
            await expectRevert(
                sonneMerkleDistributor.connect(funder).withdrawFunds(blocknr, ethers.MaxUint256),
                'Rewards may not be withdrawn',
            );
        });
        ...
        it('isClaimable() reverts for an invalid Merkle proof', async () => {
            await expectRevert(
                sonneMerkleDistributor.isClaimable(blocknr, staker1.address, []),
                'MerkleTrie: ran out of proof elements',
            );
        });
        ...
        it('isClaimable() reverts when reward is not found for given Merkle root', async () => {
            ...
            await sonneMerkleDistributor
                .connect(funder)
                .addReward(1000, ethers.hashMessage('rando'), blocknr + 1, 10000000000, 1000);

            const { key, proof } = stakers['0x90F79bf6EB2c4f870365E785982E1f101E93b906'];

            await expectRevert(
                sonneMerkleDistributor.isClaimable(blocknr + 1, staker1.address, proof),
        ...
        it('cant create a second reward with the same merkle root', async () => {
            ...
            //TODO discuss
            await expectRevert(
                sonneMerkleDistributor.connect(funder).addReward(1000, accountRoot, blocknr, unlockTime, 1000),
        ...
        describe('when unlock time is reached', () => {
            ...
            it('funder cannot over-withdraw', async () => {
                await expectRevert(
                    sonneMerkleDistributor.connect(funder).withdrawFunds(blocknr, 1001),
            ...
            it('other cannot withdraw', async () => {
                await expectRevert(
                    sonneMerkleDistributor.connect(staker1).withdrawFunds(blocknr, 100),
                    `AccessControl: account ${staker1.address.toLocaleLowerCase()} is missing role 0x7840a44bf6bbc1b45786ad46ece0694a9179b33e609d4ac4a51e6466e1f664c2`,
                );
            });
    ...
    context('when a blockNumber is underfunded', async () => {
        ...
        it('cannot claim when not enough balance', async () => {
            const { key, proof } = stakers['0x90F79bf6EB2c4f870365E785982E1f101E93b906'];
            await expectRevert(
                sonneMerkleDistributor.connect(staker1).claim(blocknr, proof),
